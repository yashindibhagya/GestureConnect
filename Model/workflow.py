...
def main():
    ...
    # Step 1: Extract keypoints from the collected videos
    run_command(
        f"{sys.executable} scripts/prepare_data.py",
        "Preparing data (keypoint extraction + augmentation)"
    )
    ...
    run_command(
        f"{sys.executable} scripts/evaluate_model.py --model models/sign_language_model.keras",
        "Evaluating model"
    )
