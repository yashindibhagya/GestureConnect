...
export default function Learning() {
  ...
  const { coursesData, isLoading, getCourseProgress } = useContext(VideoContext);
  ...
  const renderCourseCard = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.courseCard, { backgroundColor: item.backgroundColor }]}
  ...
  const handleCoursePress = (course) => {
    ...
    if (!course.signs || course.signs.length === 0) {
      console.warn(`Learning: Warning - Course ${course.id} has no signs`);
    }
